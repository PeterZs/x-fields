...
def Blending_test(coord_in,
    ...
    if args.type == ['light','view','time']:
        ...
        weight_shading_sum = tf.reduce_sum(weight_shading,0,keepdims=True) + epsilon
        weight_occ_shading = weight_shading/(weight_shading_sum)
        ...
        if args.type == ['time']:
            weight            = tf.exp(-args.sigma*w_res*dist)*(1-tf.abs(delta))
        else:       
            weight            = tf.exp(-args.sigma*w_res*dist)
